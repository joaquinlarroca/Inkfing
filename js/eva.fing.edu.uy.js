...
async function initImages() {
    inkFingProcessImages(document.querySelectorAll(".img-fluid"), "eva.fing.edu.uy/", {
...
async function initLogo() {
    inkFingProcessImages(document.querySelectorAll(".img-fluid"), "eva.fing.edu.uy/login/", {
        color1: primary,
        color2: secondary,
        point: 100
    });
...
async function treeDisplay() {
    ...
    let horizontalTabs = document.querySelectorAll("ul.format_onetopic-tabs")[0];
    if (!horizontalTabs) return false;
    let navTabsTextArray = Array.from(horizontalTabs.children, (element) => element.children[0]?.innerText);
...
if (window.location.pathname == "/") initImages();
